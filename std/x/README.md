...
| `crypto/chacha20` | 7 | **the D53 crypto ladder's second rung** (campaign stdc-crypto, entered at sc17): ChaCha20, Poly1305 and AEAD_CHACHA20_POLY1305 from RFC 8439 — the cipher that needs no tables, chosen because its constant-time story is true by construction in portable code — gated on the RFC's complete vector set (28) and the full Wycheproof `chacha20_poly1305_test.json` (256 valid + 60 invalid asserted, 9 InvalidNonceSize answered by the documented trap). In the nursery for sha2's reasons: D53's ladder is OPEN and the lane shape is uneven by mechanism (the ChaCha core's wrapping rotations are refused by the checked tier, F-0091/#130 — inherited, s111 fixing concurrently; Poly1305 is checked-int and three-lane even now). Poly1305 lives inside because a resident may not depend on a resident; the graduation move splits them, and splitting also hands the MAC its own checked lane whole. NO random-nonce surface (no entropy story; the D-question lives with the rung that needs it). | c28's ct tier exists to verify the recorded `ct:` obligations (`open`'s tag compare is its named FIRST target), or the AES-GCM rung's D-question forces `std.crypto`'s module shape — whichever ruling arrives first |
