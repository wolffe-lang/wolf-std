    ...
    for section in tomlite::parse(text, what)? {
        ...
        for (k, v, line) in &section.entries {
            let slot = match k.as_str() {
                ...
                other => {
                    return Err(format!(
                        "{what}:{line}: unknown implementation `{other}` (lupin, wolfc)"
                    ))
                }
            };
            let expect =
        ...
        let (Some(lupin), Some(wolfc)) = (lupin, wolfc) else {
            return Err(format!(
                "{what}: `{test}` must record both `lupin` and `wolfc`"
            ));
        };
        if ledger
            .insert(test.clone(), Entry { lupin, wolfc })
            .is_some()
        {
