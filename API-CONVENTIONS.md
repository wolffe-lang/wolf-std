...
acceptance gate). Once reviewed, this document is binding: every scNN review
cites it, every `pub` item conforms to it, and a deviation is either a bug
or a reviewed amendment to this file; a local style counts as neither of
those. Decisions in force: D30 (rows, no unwinding), D31 (facade + nursery),
D32 (directory = module), X1 (receiver modes), X3 (checked arithmetic), D10
(regions are the allocators). The sc01 sprint extends §2 (absence tags) and
the sc06 sprint extends §2 (tag naming and payloads), and both do it as an
...
- No `get_` prefix anywhere. Bare `get` is the one checked-access
  ...
  sentinel. Everything else says what it fetches (`len`, `first`,
  `keys`).
...
- Every `pub` signature states its error row explicitly or names a
  reviewed alias. The sc01 and sc06 sprints own the alias inventory. The
  sealing rule (s15, house law) is that a `pub` item never exports an
  unnamed inferred row.
...
- Coarsening (collapsing a precise row into a broader one) is a named
  `impl`/function the caller opts into. A std signature does not do it
...
it here. There are two recorded interims, each filed upstream and
flipped with the pin bump that fixes it (`docs/findings.md`):
...
- The helper inventory is blocked: rows exist only in return
  ...
  That was resolved at the sc06 pins. Rows parse and execute in parameter
...
the same tag. Reserved alongside it, for the sprint that ships them, is
`utf8`, for bytes that are not well-formed UTF-8 (`std.bytes.to_str`). Both
are payload-free marks, so both are lowercase by this section's rule and
both ride the `None`-spelling interim below.
...
- Every doc example writes the call-site modes (`(mut b).push(1)`) the
  way the call site must write them. Examples that hide the mode are
  doc bugs and fail review.
...
- Doc-example extraction format (defined here, consumed by the rig
  ...
  write let-bindings on their own lines. A fenced block always means
  executable code, so anything unrunnable today is written as prose
  instead.
...
row and the line documents what a caller writes at that call site:
...
**§4 note (sc02): one module per example.** The extractor imports just
the documented module (`use std.<path>`), so an example may only
...
- The operator bridge (`==`/`<=>` desugaring to `Eq.eq`/`Ord.cmp` for
  user types) has a filing (F-0004) and nothing in std assumes it:
  today `<=>` is builtin-only and yields an int in lupin, and enum `==`
  is refused by wolfc. std code writes trait calls and leaves the
  bridge alone.
...
- No std module defines a function named `assert`. A module item
  ...
  and severs the module from the trap it needs. The two-argument
...
- Every container module's header states which operations belong to
  the language and which to std, and no module assumes it: `List`/`Pool`
  construction (`List[T]()`), indexing (`xs[i]`, `p[h]`), index
  assignment, `for` iteration and `Pool`'s two-phase `reserve`/`init`
  are the language's; every named operation over them is std's. The
  split itself is a filed recommendation (F-0011) and not folklore.
- A container operation takes its container as the first parameter with
  the X1 mode the operation deserves: `mut` for mutators (`push`, `set`,
  `clear`), the bare read default for queries (`len`, `get`,
  `contains`), `take` only for true consumption. Call sites spell it:
  `list.push(mut xs, 1)`.
...
- Where the lanes disagree about a primitive, the std function decides
  ...
  The sc11 sprint completes the rule with its other half. s71 ruled the
  empty needle (`[mem.str.empty]`: count 0, one whole piece, identity) and
  every lane answers it, so the guards are deleted and the delegation is the
  contract again. That holds where the ruling disagreed with std's reviewed
  contract, which it did twice: `split("")` and `replace(s, "", …)` trapped
  `assert` in std and are defined answers in the language. std adopts the
  ruling without argument, because a std function that second-guesses a spec
  clause is a worse thing than the ambiguity the guard was covering. Two of
  the six guards survive in a different role and say so on themselves:
  `splitn` and `replacen` walk `find` instead of delegating, and `find("")`
  is 0 forever, so they state the ruled answer explicitly to reach it at
  all. The full rule is to guard where the lanes disagree, file it, and
  delete the guard the day the toolchain rules; the filing is what makes the
  second half happen.
- A code point is a `char` (sc25; D58 `[type.char]`), and the doors
  ...
  compare against `char` literals without ever computing on them, and
  `utf8_len` is total over `char`. The review gate is that no
  arithmetic touches the scalar anywhere. The `int`/byte-offset
  currency stays on `str.code_points`, `char_offsets` and `char_count`:
...
- Byte width is `List[int]` and monomorphic beats generic. `std.bytes`'
  `len`/`is_empty`/`at`/`find`/`starts_with`/`ends_with` duplicate `std.list`
  names by design: `std.list`'s are generic and therefore execute on the
  interpreter lane alone, while these are concrete and execute on all three.
  When a std function could be generic or concrete and the concrete type is
  the one programs actually hold, ship the concrete one and say why in the
  header. Two modules may share function names as long as no program imports
  both (F-0026 is a per-program linker collision), which is why `std.bytes`
  has its own producer (`from_str`) and its tests never import `std.str`.
...
needs.** The s77 wave made `s.bytes()` the receiver's own pointer and
length, read in place wherever the call is consumed. Writing eight bodies
against it settled three things.
...
- Trap where the caller broke the contract, raise where the data did.
  ...
  raises. `rand.below(0)` traps and `rand.choose(empty)` raises, and this
  rule is what separates them: the first names an empty range, the
  second finds one.
...
- A sorted-input precondition is documented, not checked. `binary_search`
  and the bound family assume ascending order and answer
  *something* on unsorted input instead of trapping, because verifying costs
...
`xs.take(2)` that left `xs` usable would be a false friend at the
...
- The format spec is a spelling; std owns the operations. Every
  ...
  same semantics. The obligation the split creates is that the spec text
  and these functions must agree clause for clause, which is why §7.4's
...
- Byte sequences are `List[int]` with a documented 0..255 element
  ...
  Every signature keeps its shape when `Bytes` lands. At sc34 the
...
- No std accessor returns an enum through an error row. The same
  ...
  The rule is retired at the sc12 pin: it was written as an interim with
  F-0037 as its exit, wolf-interp#16 closed at lupin 0.1.10 ("an enum variant
  is a value, not a raise"), and the one-line reproducer takes the value path
  on the lane that matters. The shape is legal now and the withdrawn functions
...
- Locale is never a parameter in core. Grouping separators are
...
rule this sets, for the closeout to confirm, is that a conversion belongs
to the module that owns the target representation, not to the module that
owns the source type. `std.str` keeps the string operations and no longer
...
- Coarsening is a named call the caller writes. `errors.coarsen(e)` turns a
  `ParseErr` into a `Failure`; no std signature widens a row silently, and
  there is no `From` conversion to make it implicit (D30). The precision
  given up is the ability to branch on the kind, and giving it up should be
  visible in the caller's source.
...
**§13 amendment (sc09): the rig denies warnings.** A non-empty `warnings` array
in the observation record (`[proto.record.warn]`) is a RED in
`cargo xtask std-test` and in `doc-examples`, on every lane that reports one.
`conform-run` still rejects `--deny-warnings` (F-0046, re-verified at the sc09
pin), so the rig does it: the flag this repo asked for twice is approximated by
the one signal the protocol does give, and both executing implementations
populate it now (wolfc since s67, lupin since 0.1.6's lint wave). It paid on
its first run: three doc examples carried `0.0 - x` sites that W0402 had been
flagging into a void. There are two limits, stated so nobody reads more into a
green rig than it says. The array covers the entry file only, so a warning
inside a std module body is invisible from here (F-0053's open half), and a
lane with no lint tier reports nothing at all, which is not the same as
reporting clean.
...
- Every os-facing `pub` item names its capability, in its own doc, by
  ...
  path helpers) says so, in the words "pure, comptime-safe",
...
- A capability module's lanes are unequal, and the rig says so
  ...
  least one lane must reach `exit(0)`, and a stated refusal on the others
  is acceptable. That is a stronger requirement than "the reference
  machine runs it" for the modules where the reference machine
  cannot.
...
- A tag a program cannot reach is documented as unreachable, not omitted.
  ...
  surface changes. Removing it would be a lie, because the row is the
  toolchain's.
...
- The pure member of a capability module is stated twice: on the function
  ...
  are comptime-safe, just as `std.fs`'s path helpers are. In an os
...
**§14's last rule, restated because the sc08 sprint paid it twice.** "What a
capability module may not do to get a green lane" now has two worked refusals,
blocked by the same finding from opposite directions: `std.io.input_all`
(sc07) and `std.net.read_all` (sc08, written and withdrawn inside the sprint).
A loop over a rowed read must stop on one tag and re-raise the others, and no
handler can tell them apart. At the sc08 pin the shape that should
discriminate (`else |e| match e { … }`) compiles, runs, and matches its first
arm for every tag on the executing lane, while the other two lanes get it
right (F-0052). The rule that decides both is that a capability module ships
no operation whose failure mode it cannot distinguish, and the version that
"works on the happy path" is the version that loses someone's data.
...
- A facade over a scalar ABI exists to make unit confusion a compile
  ...
  follow is that when a builtin tier hands std a bare `int` whose unit is the
  whole contract, std names the unit in a type, never in a parameter name. It
...
- Rendering is a documented posture, not an apology per function. A
  formatter in an os module states once, in the header, which profile
  it writes and what it has not got. `std.time`'s is: RFC 3339
...
- A pure builtin family is not a capability, and it still does not get a
  ...
  capability modules ("at least one lane must reach `exit(0)`, and a stated
  ...
  distinction that survives is that a capability lane may be dark forever by
  design (lupin has no sockets), where a tier lane is dark until someone
  writes the mirror.
...
- The graduation clock ticks at campaign closeouts and has three
...
- Check a resident's dependencies against its own lanes before writing a
  ...
  written and withdrawn inside sc10 for that (F-0061).
...
- The row vocabulary is the toolchain's, for the fourth time, and it gains
  ...
  three), `{signal, io}` for waiting on one, `{io}` for killing one.
  `signal` is new to §12's inventory: the child died without an exit code.
  It is a payload-free mark by §12's casing rule, and it is not a
...
- When the language owns the verb, std takes the next word, and says
  ...
  spelling (F-0062), just as `copy` made `std.fs.copy_file`. The rule this
  sets is that a name collision with a keyword is a paragraph on the
  function, never a doc-free rename, because the reader arriving from
  another language types the keyword first. The same paragraph is what keeps
  a std verb from competing with a language concept. A child process and a
  task are different failure models and should not share a word.
- A builder is pure, and in a capability module it is stated three
...
- A capability whose happy path cannot be witnessed portably says so, on
  ...
  states that the exit-code path rests on the toolchain's own
  unix-gated tests. This is a weaker standard than §13's "a claim about what
  an implementation answers is a test or it is a rumour", and the response is
  to name the gap in three places instead of letting a green rig imply
...
would unblock it, and ships the function in the sprint after that finding
closes.* sc13 is the first sprint to be on the receiving end of it twice, and
both halves are recorded here because they show the clause working rather
than being invoked.
...
- Re-armed. F-0057 (nothing builds a `str` from a number) closes at this
  ...
  encoder/decoder pairing, refused for four sprints). They are named in four
  headers instead of one closeout, so the debt lives where the function is
  missing.
...
- A pure tier gets its row vocabulary adopted verbatim too. §14 wrote the
  rule for capability tiers (fs, io, net, process) and the sc10 sprint
  extended the lane-honesty half to pure ones (`json_*`). `to_str`'s `{utf8}`
  is the third case and the cleanest: the row is `str_from_utf8`'s, unrenamed
  and uncoarsened, and the test that proves it is not a handler asserting the
  tag but the record naming it (`error: utf8`, exit 1). A wrapper that
  renamed or widened a row would look identical from inside a handler, which
  is why the propagation test is the one that matters.
- When a std function wraps a validating primitive, its test walks the
  primitive's whole refusal surface through the facade. Not a sample, but
  every named class, plus the accepted rules that are easy to lose (an empty
  input, an interior NUL, the boundary value adjacent to the first invalid
  one). And where std already had an independent implementation of the same
  predicate (`bytes.is_utf8`), it keeps it and a test holds the two to the
  same answers. Delegating the predicate to the primitive would have made the
  agreement a tautology and cost the predicate a lane; two implementations
  that must agree is a stronger arrangement than one, and this is the case
  that showed it.

**§14 amendment (sc14): the debt clause paid in full, and what a contract
is worth when it comes due.** The sc13 sprint re-armed the clause on four
functions and named them in four headers. The sc14 sprint ships all four
(`json.parse`, `json.unescape`, `json.escape`'s totality, `hex.decode_str`)
with no extension and no renegotiation, which makes this the clause's first
complete cycle: armed, named where the function was missing, and paid on the
date. Four rules came out of the payment.

- A contract written as a specification survives its wait; a contract
  written as an apology does not. Every signature written for these four at
  sc05 and sc10 was still right, except where a measurement had moved under
  it, and both movements matter. `unescape`'s tag changed from `parse`
...
- A blocked lane is not a blocked function until you check whose lane it
  ...
  after it was filed. The rule for the register is to file the mechanism and
  not the symptom: "this machine cannot build a `str` from bytes" is what the
...
cannot see the difference.** The sc08 rule was that a wildcard claiming
nothing is the right handler when a module cannot act on the difference
between tags. F-0079 adds the harder half **[F-0079 closed at lupin 0.1.13
...
the toolchain's word and not this repository's. That second habit is what
...
- A second family is a second module, not a suffixed verb. wolf has no
  ...
  `std.net`. That is §1's rule, and the same one that put the `f64` family
  in `std.math.float`. The alternative spelling (`net.listen_unix`) was
  ...
  not change. The rule is that a new address family adds address calls and
...
- The tag exists because a bare `io` is unactionable, and that is the
  argument for any new tag. `[os.net.unix]` was filed
  (wolf-lang#227) so a program could tell "this machine has no
...
- An acquisition call adds one function and nothing else. A new way
  ...
  same rule `std.net.unix` follows (the address family differed there, so
  the address call was the only thing that module added) generalised to the
...
- Where the kernels differ in degree rather than in row, document
  ...
  on both linux and macOS and behaves differently on each, since linux
  distributes accepts across the group by a 4-tuple hash and macOS hands
  ...
  both postures and promises the two guarantees both hosts make (every
  dial is accepted by some member; the survivor takes every dial after
  the others close), and the witness asserts those two and nothing about
  which member. A test that pinned the developer's host's scheduling
  would pass here and fail on the other tier-1 host, and would have
  encoded a kernel's policy as the language's contract.
- A tier's refusal is a stated posture, not a gap, when the tier says
  why. The checked machine refuses `net_adopt_listener` and a non-empty
  `os_spawn_with`, naming the construct, because it is the `wolf` binary
  interpreting a program: a descriptor handed to "the program's child"
  would be handed to the compiler's child. That is
  ...
  **Probe it with `wolf conform-run --checked` and never with `wolf run
  --checked`**, which runs the native build: the sc37 sprint nearly filed
  a false spec divergence off the second spelling.
...
- A module may exist for one function when its subject is its own.
  ...
  namespace of its own to a function filed under a neighbour whose header
...
- A row that exists to prevent a silent default must not be defaulted
  by std either. `os_cpus` answers `io` rather than a quiet `1` so a
  program resolving "workers auto" can say it did not learn the number.
  `std.os.cpus` therefore propagates `io` and supplies
  ...
  distinction the clause was written to create; the witness asserts
...
- A typed container is right when the set cannot mix, and the
  ...
  on both, and here it is written on both. The day a sum type exists, they
  change together instead of one at a time.
...
- A refusal can be a row on one host and a whole-program verdict on
  ...
  it runs, and the only correct shape for the hosts where it is not.
...
- 2026-09-02, sc32 (§2's trap rule and §12's mark casing, applied to a
  ...
  trap test the way `bytes.slice`'s is. `cap: 0` is not in
...
- 2026-08-30, sc29 amendments (the net byte tier + the deadline, and the
  ...
  single thread, with the shape stated in the module header as an API
  decision; every adversarial input a named row (the negative battery, pure
  and three-lane); the CertificateVerify verify a dispatch that answers
