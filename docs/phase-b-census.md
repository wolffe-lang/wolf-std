...
- `read_line` → `input_line`. A module item that shares an ambient
  ...
  lupin, a facade in one machine and unbounded recursion in the other,
  with no diagnostic (F-0047). std does not build on an unruled
  resolution order.
...
includes the two rig-facing asks: stdin injection through `conform-run`,
...
Dispatch is the blocker here, the same one that holds 16 Phase-A
functions.
...
spell `else copy empty`. The nine new fs/io rows land on top. The
...
The vocabulary is new this sprint and should stay rare, since a
nondeterministic verdict is a conformance defect.

All nine sc07 tests are `unsupported` under lupin by design: the
interpreter has no filesystem, no line read and no stderr writer. The rig
records that posture once (`CONTRIBUTING.md`, the sc07 section of
`tests/ledger.toml`) rather than in nine file headers.
...
Two names differ from what the sprint contract listed, and a measurement
produced both:
...
- F-0049 — the net builtin tier · 5 contracts: deadline helpers
  (`set_deadline`/`read_timeout`, where the `timeout` tag is declared and
...
- F-0044 (6), F-0046 (1) and F-0004 (2) are unchanged from sc07's table.
...
`std.unicode`, `std.fmt` and `std.hex`. The sc09 sprint lands 20 of
the 30:
...
- `std.str`: 12 of 15. `get`, `find`, `rfind`, `count`, `split`,
  ...
  `graphemes`, all three of which need a `char` type or segmentation
  tables rather than a primitive (F-0018's surviving clause).
...
bodies, as `std.unicode` planned its own.
...
allows one home per concept. `std.str`'s header says so instead of
...
all-three block a sprint has landed, and the reason is structural: this
code is monomorphic functions over `str`, `int`, `bool` and
`List[int]`/`List[str]`, and the four things that darken a column
...
wave, with lupin held at 0.1.6). This is the first sprint whose surface
comes almost entirely from one upstream merge: the s40 wave landed
fourteen builtins in three families and the sc10 sprint wraps three of
them.
...
- `std.time`, 24 functions. Six reach the clock (`now`, `unix_ms`,
  ...
  floor-division correction spelled out.
- `std.env`, 8 functions. `args`, `arg`, `get`, `has`, `vars`, `set`
  (capability `env`) plus the two pure `K=V` cutters. This is the module
  the bs10 sprint asked for: a project can stop holding its input as data.
- `std.x.json`, 11 functions. `is_valid`, `get`, `type_name`, `len`,
  `has`, the typed reads `str_at`/`bool_at`/`is_null_at`/`int_at`, and the
  two pure path builders. It needs no capability.
- Two of the four census rows the sc09 sprint re-owned:
  `std.fmt.truncate_to` and `std.hex.encode_str`, both three-lane, both
  one small body over a primitive that landed at s37. `std.fmt` now has
  no blocked contract at all.

### What did NOT land, and why — the other two census rows

The sc09 sprint re-owned four rows on the strength of F-0018 retiring,
and predicted all four were writable. Two were. The other two are
`std.json`'s, and the sprint that took them found a different finding
standing behind the one that retired:
...
- `json.unescape` is blocked by F-0057 and not by F-0018 either. The walk
  ...
  `str`, with no `from_utf8`, no `char`, no `strbuf.push_byte`. An ASCII-only
...
- `json.escape`'s one refusal is the only one of the four that is simply
  unwritten, and unwritten by choice: making `escape` total changes
...
Here is the census correction, and it is the most transferable thing in
this sprint: "the blocker retired" is not "writable" until every finding
on the signature has been re-measured, not just the one that was in the
way last time. The sc09 sprint predicted four writable functions from one
...
that advanced nothing already recorded. That is what the wave was: new
builtin families std had never wrapped (s40), a fixed shape std had
...
the F-0052 witness), six are two-lane (every capability test, since lupin
has no `time_*`/`env_*`, which is a pin drift rather than a design
refusal), one is two-lane the other way (`x/json/path_helpers.lu`: lupin
and checked, native dark), and three are one-lane (the json queries,
checked only). A pure member always buys a lane, and separating it into
its own test file is now house habit.

### The blocked inventory

Unchanged from sc09 except for the movement above: F-0049 (5), F-0050 (1),
F-0044 (6), F-0046 (1), F-0004 (2), F-0057 (1). F-0052's two
(`io.input_all`, `net.read_all`) are no longer blocked, since the finding
closed, and are unwritten pending a sprint that owns them; their module
headers still say "blocked" until one does, which is a debt this census
names. F-0058 adds 1 (`x.json.keys`, blocked on a query tier with no key
...
killing, with `signal` new to §12's inventory.

Two names differ from the obvious ones, both for reasons the contract could
not have known. It is `start`, not `spawn` (`spawn` is one of the 50 keywords,
opening a task and a `spawn proc`, so `pub fn spawn` is `E0008`, F-0062), and
...
- `io.input_all` (sc07's worked refusal) and `net.read_all` (sc08's,
  written and withdrawn inside that sprint) are both shipped now, four
  lines and one handler each. Neither became writable because std lowered
  its standard: both needed a loop that stops on one tag and re-raises the
  others, and both waited for s70's match tier (F-0052) and s71's
  payload-pattern ruling (F-0043). §14's refusal rule now has its first two
  retractions and a third clause: ship the function in the sprint after the
  finding closes.
...
- `bytes.to_str` is still a contract, re-documented after four probes:
  `str.from_utf8` does not resolve, `strbuf.push_byte` is not a method,
  `'h'` is `E0101` at the lexer, there is no `bytes_to_str`. wolf-lang#58
  is open; F-0057 is unmoved for a third sprint.
...
The three rewrites moved no row because the answer changed while the
...
the sc09 sprint wrote the file instead of the sentence.

The s73 wave moved no row at all. Native concurrency is the wave this
sprint was told to ripple, and std wraps no concurrency surface at this
pin: no task facade, no channel helpers, nothing that names `spawn` (which
is a keyword, F-0062). So the ripple is entirely documentary, and it is
real: every sentence in this repository that budgeted a concurrent program
to the interpreter lane is now false, `std.time.sleep`'s doc had to say
that a thread-blocking sleep now stalls sibling tasks on three rungs rather
than one, and `std.process`'s "kill it from another task" advice became
writable advice instead of a one-machine trick. Zero rows, three
...
drift closes on a release; a posture closes on a decision.
...
the module's central claim, that a child's exit code comes back, has no
portable witness in this repository at all.
...
own conformance pin `613c3dc` being an ancestor of the wolf sha. That is the
...
(`tests/fmt/decimal/shortest_round_trip.lu` timed out at the rig's
60-second per-test ceiling) after three `std-test` runs had passed it in the
same hour. The re-run was green and so was the run after it, and the flake is
...
a sprint here can say that. The subject was what the s77 wave made cheap.
...
not a digit), and the probe loop it required is gone.

### What did NOT get rewritten, and why

- `std.bytes`, all nine functions. Their first parameter is `List[int]`,
  and an argument is the position s77 materializes in, so
  ...
  library cannot fix it from its side: F-0072 (wolf-lang#86) is the ask, for
...
CI timeout that turned into a bisect (F-0074), and it belongs here because
it is the only direct evidence that the rewrites do anything:
...
reads a byte. Two caveats: both versions remain superlinear in the input
for a reason that has nothing to do with either probe (F-0074: `List.push`
is O(n) per push on this lane, and `decode` pushes a byte per pair), and
this is one input on one lane on one machine. It is an anecdote with a
mechanism rather than a benchmark, and it is labelled as one.
...
best candidate of all. Its only executing lane is the interpreter, since
the f64 body is `unsupported` on both compiler rungs (F-0026, F-0061), and
there the view materializes, so the rewrite measured neutral-to-slower
against a probe that was already cheap (no `.upper()`, ten single-byte
slices). It was reverted inside the sprint. Check which lane runs the
module before rewriting it onto a primitive whose win is native-only: a
one-lane module whose lane is the interpreter gets nothing from s77, and
`std.fmt.decimal` is the whole class.
...
Every flip is listed, and there are none. Not one of the 183 rows this repo
...
whole difference above is the two rows the sc12 sprint adds:
...
changed cost rather than depth. That is worth stating, because a
...
One block leaves the inventory, and it is the biggest one here: F-0037 (an
enum returned through an error row takes the miss path on every call) is
...
first time. The sc12 sprint does not write it: it is not in this sprint's
contract, and sc10's rule applies to itself, since "the blocker retired" is
not "writable" until every finding on the signature has been re-measured.
The sprint that takes the row owes F-0039 (nested rows) and F-0029
(cross-module enum consumption) a fresh measurement first. API-CONVENTIONS
§11's "no std accessor returns an enum through an error row" was written as
an interim with F-0037 as its exit, and the exit has arrived.

## 11. sc13 — two closures spent, in opposite directions

Pins: wolf trunk `4e316ad` (three waves past sc12's `f8dca42`: s79 bench,
s80 token audit, s81 str equality, the three the sc12 sprint named as in
flight and did not chase), lupin 0.1.11 with its own conformance pin at
...
been a single sprint. Both ritual gates green in a clean scratch clone at
...
the signature has been re-measured) applied to itself. The sc12 sprint named
the two findings the json sprint owed a measurement, and both were taken
first:

- F-0037: `fn id(v: V) -> V ! {none} { v }` prints `value path wins 7`
  under lupin 0.1.11. Closed, and now spent.
- F-0029 is unmoved. An enum value crosses a module boundary (and now
...
- F-0039 is unmoved. `int ! {none} ! {none}` runs under lupin and is
...
sc14, and both module headers say so with the clause named. That is why
this is a section rather than a note.
...
Every flip is listed, and there are none. No row this repo carried into the
bump moved in any direction; the whole difference is the four rows the sc13
sprint adds:
...
not leave, since `fs_read_bytes` (F-0044) is still unwritten, so a byte read
has no producer to hand `to_str`, and the two halves of that pair have now
been waiting for each other for six sprints.
...
Pins: wolf trunk `27cdb14` (the s84 merge, three waves past sc13's `4e316ad`:
s82 lane coverage, s83 the ABI inventory, s84 allocation-free iteration),
lupin 0.1.12 with its own conformance pin at `4e316ad`: the interpreter
...
exit codes written to a file as they happened, on the second attempt, with
the first attempt recorded rather than hidden (`vendor/tools.toml`: gate 2's
...
there are none left in either module. The sc13 sprint wrote the debt into
four headers rather than into one closeout, and this is what that bought:
nothing had to be rediscovered, every signature was already specified, and
the only surprises were measurements.
...
had zero lanes, which is why `std.x.json.float_at` was withdrawn
...
already trust) applied to a parser. 86 documents went through
`std.json.parse` and through CPython's `json`, compared on acceptance and,
where both accept, on the value read back: 81 agree exactly, and all five
disagreements are the reference being lenient where RFC 8259 is not.
...
nesting cases. The harness is 40 lines of Python driving `lupin` and
diffing; nothing about it is wolf-specific, and running it before writing
the tests would have been better still.
...
the sc13 sprint added it with `bytes.to_str` as its only entry and
documented it as dying at the release that resolved the primitive. This one
emptied on schedule, as `WOLFC_WAIVERS` did at sc03.
...
were green, exit 0 both times), which is sc13's rule holding a second time:
once the mechanism is named, a red run under load is a scheduling fact, so
run the rig on a quiet machine, state the load, and do not shrink the test.
...
wolf trunk `27cdb14` -> `02c1e88`, lupin unchanged at 0.1.12. This is the
...
`List[int]` takers are unchanged as source; the s89 wave made eight of them
lendable, which is a property of their bodies rather than of their
signatures, and the module header now explains the verdict (`to_str` is the
ninth and materializes because it hands its parameter to a builtin). The doc
examples moved to `"…".bytes()` because that is the call a reader should now
write.
...
- Three marks joined the inventory (`exists`, `invalid`, `cross_device`),
  adopted verbatim, which is §14's rule for the fifth time.
...
The witness is a byte rather than a stopwatch.
`tests/fs/binary_round_trip.lu` writes `00 80 FF 0A C0`, reads it back,
copies it, moves it, and checks every byte at every step, with `read_text`
on the same file asserted to miss, so the file is provably not text. It
could not have been written at any earlier pin: nothing could put an `0x80`
in a file, and `copy_file` would have refused it.
...
is the only way to attribute them. The nine new rows: six `fs`
...
delivered. The contract asked for atomicity; the language does not promise
it, because POSIX replaces a destination atomically and windows
`MoveFileEx` does not, so upstream ships `fs_rename` claiming the effect
and no `fs_rename_atomic` at all. std adopts that reading rather than
re-promising one level up. A contract can be answered by a decision instead
of by code, and the record of that is a withdrawal with the reason.
...
cites a closed finding is the failure mode the sc13 sprint named.

### Findings

Five close (F-0044, F-0045, F-0071, F-0072, F-0076) and two are filed:
...
- F-0081: lupin declines the s38 fs names with a sentence and refuses
  ...
  drift, which has twice turned out to be the question that mattered here
  (F-0070, F-0075).
...
stays open because its other two legs hold.

## sc13 note — no dyn surface, on purpose

D47 made `v as dyn Trait` real and the sc13 sprint probed it green on
every lane, then shipped no dyn API. Every candidate (an erased
predicate holder, a dyn comparator slot) has a generic spelling that is
strictly better at these pins: no allocation question, no place-only
constraint on the caller, and monomorphized dispatch where a vtable
