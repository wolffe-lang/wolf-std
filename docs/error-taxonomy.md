...
sc06 made. The table and the last section also carry sc07's io tier
(`std.fs`, `std.io`), the first sprint to spend the `eof`/`utf8`
reservations and to add tags std cannot test.

The measurements come from a re-runnable script over `pub fn` signatures
in `std/`, so anyone can check the numbers in the alias filing (F-0041)
against the tree.
...
third tag yet. The sprint contract asked which signatures exceed three
tags; none do, and the alias surface is filed on the io taxonomy's
behalf (F-0041 carries the argument and these numbers).

## Every tag, its sites, and its verdict

Verdict key: "conforming" is a lowercase mark with one failure mode a
caller can act on and no payload needed; "renamed" was nonconforming at
sprint start and fixed in this sprint's rename; "watch" is conforming
with a recorded question for a later sprint.
...
The rename ran tree-wide. `None` → `none`, `Done` → `done`,
...
went in as a separate mechanical commit, which is this one. It cost
nothing: the whole rig of 127 tests, 211 doc examples and three lanes is
byte-identical either side of the rename.
...
(`Parse(ParseErr)`), and the audit recommends they stay the only ones
until a caller can act on the difference.

The `parse` retrofit is costed and left for later. Turning
`hex.decode -> List[int] ! {parse}` into `! {Parse(ParseErr)}` would let
a caller say where the input stopped being hex, a real improvement. It
is not in sc06, for two costs: the tag's payload type has to come from
`std.errors`, so every module that gains the import gains its lane story
too, and the change is API-breaking in the direction that matters least
(a caller who only wants "it did not parse" must now destructure). The
recommendation on record for stdc02 is to retrofit `parse` when the io
tier gives a second caller who branches on position, in one commit
across `fmt`, `hex` and `base64` so that the family stays one shape.
...
3. One tag per failure mode a caller can act on. Call sites and internal
   causes do not each get their own.
4. Absence (`none`) is not an error and never carries a payload.
5. Coarsening is a named call the caller writes (`errors.coarsen`).
...
7. `errdefer` is the cleanup form on the error path only. In Phase A no
   ...
   recorded and unused. The first `errdefer` in std will be in the io
   tier, releasing a handle it opened.

## The io tier's five tags (sc07, Phase B opens)

`std.fs` and `std.io` add five marks and adopt them verbatim from the s38
builtin tier: `not_found`, `denied`, `io`, `utf8`, `eof`. std could have
defined its own error type over the builtins, so the adoption is a
decision and is recorded as one. Three reasons. The tags are already the
...
caller through std without widening, so `std.fs.read_text` costs the same
as `fs_read_text`.

What the tier reveals about §12's rules:

- Rule 3 (one tag per actionable mode) earns its keep here. `io`
  covers every host failure outside "missing" and "refused", and no
  caller can act on the difference between a full disk and a broken pipe
  in wolf today: there is no retry policy and no free-space query. The
  day one can, the tag gains a payload and the vocabulary keeps one tag.
- Rule 2 (a payload is data) has a blocked landing. `NotFound{path}`
  ...
  rejection (F-0043); a payload nobody can destructure reads as progress
  while giving a caller less than a mark does.
- Two tags ship documented and unobserved. `denied` needs a
  ...
  writable from wolf at this pin (F-0044). The error-tier counterpart to
  §10's "accuracy is a measured contract" is this: a tag std cannot
  witness says so on the function and in the test header, and no test
  claims otherwise. (One of the two came off this list at the sc12
  ...
  `cross_device` joined it, a tag `move_file` handles but never raises,
...
- An absence tag ends a loop, and an error tag is what a loop re-raises.
  `eof` and `closed` are the same shape in two vocabularies, and it paid off
  at sc11: `io.input_all` and `net.read_all` are one loop each that stops on
  the absence tag and re-raises everything else unchanged, so neither tag
  appears in either function's row. The rows are the caller's business and
  the absence is the function's, and that split is writable only because a
  handler can now tell them apart (F-0043/F-0052, both closed).
- `signal` is the taxonomy's first "no value at all" error. Every other
  ...
  happened and produced no number. A child ran, so nothing is missing and
  the tag is no absence, and it must not be a code, because every invented
  code collides with a real one. Rule 4 covers its future: the day the
  builtin says which signal, the tag gains a payload.
- A tag with no witness is still documented, and sc11 has two of them:
  ...
  (F-0066). The error-tier counterpart to §10's "accuracy is a measured
  contract" applies here too: a tag std cannot witness says so on the
  function, in the test header and in the census, and no test claims
  otherwise.
...
- `boundary` is retired: 3 sites → 0. It was `json.escape`,
  ...
  different reason than the one predicted: `escape` wanted a byte walk
  instead of the code-point primitive, and the four characters JSON escapes
  are ASCII. It is the first tag this library has removed, at a cost of
  eight row annotations plus one test file. An interim tag that outlives
  its condition costs more than no tag at all, because by then callers have
  written handlers for it.
- `syntax` arrives: 2 sites (`json.parse`, `json.unescape`). It says "this
  text is not a JSON document", one actionable failure mode, payload-free
  by §12's casing rule, and it will gain a byte offset as a payload when
  §12's payload conventions reach this module. It is spelled `syntax` where
  the sc05 contract said `parse`, because `std.json` now declares
  `pub fn parse` and a tag that shares a name with anything in scope
  resolves to that thing, silently (F-0036, re-measured live under lupin at
  this pin). The rule that generalizes: check a new `pub fn` name against
  your module's tags, and a new tag against the module's functions.
- `overflow` gains a site (`json.parse`, 5 → 6 including the nursery's
  `x.json.int_at`): a JSON number whose magnitude has no `f64`. It is not
  `syntax`, because the document is well-formed and saying otherwise would
  misreport the caller's data, and it is not a silent infinity, which
  `stringify` would render back as `null`.
- `parse` and `utf8` each gain a site (`hex.decode_str`, whose row is
  ...
  would report bad hex to a caller whose hex was fine. The `utf8` half is
  `str_from_utf8`'s row arriving verbatim through `std.bytes.to_str`
  (§14's rule for a pure tier), so the test that matters rides it out of
  `main` instead of asserting it in a handler.
...
A row left three signatures, the second time std has removed one (after
`boundary` at sc14). `append_text`, `copy_file` and `move_file` all
carried `utf8` because all three decoded: the append read the file back,
and the copy and the move were `read_text` + `write_text`. None of them
decodes now, so none of them can raise it. §14's rule cuts both ways: a
function's row is the union of what its delegates raise minus what it
handles, and a tag that survives the delegate that raised it leaves a
caller writing a handler that never fires.

One tag is left out of a signature it could have been in.
...
arm instead of widening the signature. The same tag is on `write_bytes` and
`write_chunk`, where the list is the caller's. Whose data it is decides
whether a tag is reachable, and the two functions sit four lines apart so a
reader can see the difference.

## sc17 (06-crypto): one mark from the AEAD surface

`std.x.crypto.chacha20` ships one row, on one function, and the discipline
around it is the module's error story:
...
before the first keystream operation. `seal` has no row: sealing fails
only on the caller's own inputs, never on data.

## sc32 (15-the-budget): one mark for a failure std does not detect

`std.mem.budget` ships one row, and it is the first row in this
...
There is no payload, and since a reader will reach for one, here is why.
"How much did it want?" is the obvious field, and the contract makes it
unobservable: `[mem.region.cap.3]` makes teardown free-then-deliver, so
the breaching proc's regions are reclaimed wholesale *before* the exit
reason reaches the join, and no postmortem query can see the dead proc's
charge. `live()` at the join is already back to its pre-call reading.
That half of the contract makes the row safe to act on, and it also
leaves no number to carry. A payload here would have to be invented,
which §12's "a payload is data, never a rendered string" forbids. The
budget the caller passed is the caller's own value and needs no return
trip.

One failure is kept out of the row vocabulary. A negative budget is
`trap(assert)` at the door and `exhausted` never sees it.
`[mem.region.cap.2]` already rules it a
`trap(alloc-contract)` at the creating site, but `with_cap` creates its
region inside the proc, so that trap would be contained and would reach
the caller as a recoverable row, answering a caller's arithmetic mistake
with a value they can handle. §2's trap rule wins: the check runs before
anything spawns, and `tests/mem/budget/negative_cap_trap.lu` holds it.
`cap: 0` sits outside this family, since the clause makes it a legal
budget every charge breaches, so it answers the row, and
`breach_is_a_row.lu` pins that half beside it.
...
This row differs from every other one in this document, and the
difference is written on the function. Each of the other os tags varies
with something the caller passed or something that happened:
...
always, for every argument. That turns correct handling into a startup
decision and a fallback transport, so `std.net.unix`'s header and both
functions say "the host" in the tag's own sentence. The general rule went
into API-CONVENTIONS §14's sc36 amendment: when a surface carries both
kinds, say which kind each row is.

Two rows in the set are beyond any test in this repository, and §14's
rule says to state them. `unsupported` is windows's answer at this pin
and this rig's implementation lanes are dark on windows, so the tag is
held by construction instead: every unix witness branches on it and
prints relations that hold vacuously, the construction
`corpus/net/unix_echo.lu` uses upstream. `denied` needs a directory the
caller cannot write and there is no `chmod` builtin. It was measured out
of band this sprint (a mode-000 directory answers `denied` at bind and at
dial, byte-identical on all three lanes; the findings register carries
the reading), and it is declared in both signatures because the
vocabulary is the toolchain's.

One condition std promises no tag for. A dial of a path that names
something which is not a socket at all is neither of the two dial cases
`[os.net.unix]` rules, and the two hosts answer differently: macOS
returns `ENOTSOCK`, which arrives as `io`; linux returns `ECONNREFUSED`,
which arrives as `refused`. A CI runner surfaced it during is36, after
every developer machine had agreed on `io`, and the reference
interpreter's own test now reads the row and accepts either. This
document's rule for that shape: when the tag is the kernel's and the
kernels disagree, promise the class and leave the member open. It is a
row and never a trap, and both halves of that are worth as much to a
caller as a tag would be. `refused` keeps its own meaning, the one an
